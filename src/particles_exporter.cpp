...
void ParticlesExporter::toVtu(const fs::path& path, const double& time, const double& n0ForNumberDensity) {
    ...
    dataArrayBegin(ofs, "3", "Float64", "Position");
    for (const auto& p : particles) {
        ofs << p.position.x() << " ";
        ofs << p.position.y() << " ";
        ofs << p.position.z() << endl;
    ...
    for (const auto& p : particles) {
        ofs << p.velocity.x() << " ";
        ofs << p.velocity.y() << " ";
        ofs << p.velocity.z() << endl;
    }
    dataArrayEnd(ofs);

    dataArrayBegin(ofs, "1", "Float64", "Angle");
    for (const auto& p : particles)
        ofs << p.angle << endl;
    dataArrayEnd(ofs);

    dataArrayBegin(ofs, "1", "Float64", "Angular Velocity");
    for (auto& p : particles)
        ofs << p.angularvelocity << std::endl;
    dataArrayEnd(ofs);
